...
const emit = defineEmits(['onHandleOk', 'onHandleRemove', 'onHandleEdit']);

const todoStore = useTodoStore()

const todoNotCompleted = computed(() => {
    return todoStore.notCompleted
})
...
const handleEdit = (id) => {
  emit('onHandleEdit', id)
}

const handleRemove = (id) => {
  emit('onHandleRemove', id)
}
</script>
...
<template>
  <a-row :gutter="[{ md: 24, lg: 32 },{ md: 16, lg: 24 }]">
    ...
    <template v-else>
      <template v-if="todoNotCompleted.length > 0">
        <a-col :xs="24" :sm="24" :md="8" :lg="6" v-for="todo in todoNotCompleted" :key="todo.id">
          <a-card size="small">
            <template #extra>
              <a-space>
                <TheButtonEye :id="todo.id" @buttonEye="handleOk" />
                <TheButtonEdit :id="todo.id" @button-edit="handleEdit"/>
                <TheButtonDelete :id="todo.id" @button-delete="handleRemove" />
              </a-space>
            </template>
            <a-card-meta>
              <template #title>
